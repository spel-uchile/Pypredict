sudo apt-get install python3.6-dev
sudo apt-get install python3.6-tk
#sudo apt-get install python3-pyorbital
#sudo apt-get install python3-matplotlib
#sudo apt-get install python3-scipy
sudo apt-get install libjs-proj4
sudo apt-get install libproj-dev
#sudo apt-get install libgeos-dev
sudo apt-get install python3-pip
sudo -H pip3.6 install --upgrade pip
sudo -H pip3.6 install --upgrade pyorbital
sudo -H pip3.6 install matplotlib==2.2.4
#sudo -H pip3 install --upgrade matplotlib
sudo -H pip3.6 install --upgrade scipy
sudo -H pip3.6 install --upgrade cython
sudo -H pip3.6 install --upgrade numpy
sudo -H pip3.6 install --upgrade geos
sudo -H pip3.6 install --upgrade shapely
sudo -H pip3.6 install --upgrade pyshp
sudo -H pip3.6 install --upgrade six
sudo -H pip3.6 install --upgrade Pillow
sudo -H pip3.6 install --upgrade Fiona
sudo -H pip3.6 install --upgrade pykdtree
sudo -H pip3.6 install --upgrade cartopy
#sudo apt-get install libtcmalloc-minimal4
#export LD_PRELOAD="/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so.4.3.0"

